<template>
  <span class="navbar-text">
    <button class="btn selectable text-secondary lighten-30 text-uppercase my-2 my-lg-0" @click="login"
    ...
    <div class="dropdown my-lg-0" v-else>
      ...
      <div class="mx-3 text-secondary lighten-10 mt-5 mb-3 text-center">{{account.class}}</div>
      <div class="mx-3 text-secondary lighten-30 my-3 text-center">{{ account.name || user.name }}</div>
      <div v-show="account.graduated == true" class="mx-3 text-secondary lighten-30 my-3 text-center">
        <h1>🎓</h1>
      </div>
